 ...
 */
export const useDecorationsObserver = (navigator, decorations, onDecorationActivated) => {
    // Track which groups have had listeners attached
    const activeGroupsRef = useRef(new Set());
    useDeepCompareEffect(() => {
        ...
        if (typeof navigator.applyDecorations !== 'function') {
            console.warn('Web navigator does not support decorations API yet. Decorations will not be rendered.');
            return;
        }
        ...
        Object.entries(decorations).forEach(([group, decorationList]) => {
            ...
            navigator.applyDecorations(navigatorDecorations, group);
            ...
            if (!activeGroupsRef.current.has(group)) {
                ...
                if (typeof navigator.addDecorationListener === 'function') {
                    navigator.addDecorationListener(group, (event) => {
