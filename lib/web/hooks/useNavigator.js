...
export const useNavigator = ({ file, onLocationChange, onPublicationReady, container, onPositionChange, requestConfig, allowedDomains, }) => {
    ...
    const onLocationChangeWithTotalProgression = useCallback((newLocation) => {
        ...
        if (!totalProgression) {
            ...
            const intraChapterTotalProgression = newLocationProgression / readingOrderCount;
            totalProgression =
    ...
    useEffect(() => {
        ...
        async function initializeNavigator() {
            ...
            const nav = new EpubNavigator(container, publication, listeners, positionsArray, initialPosition, // Pass the initial position
            configuration);
            await nav.load();
            if (cancelled)
                return;
